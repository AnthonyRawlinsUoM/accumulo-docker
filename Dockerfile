...
RUN curl -s http://mirror.cc.columbia.edu/pub/software/apache/zookeeper/zookeeper-3.4.10/zookeeper-3.4.10.tar.gz | tar -xz -C /usr/local
RUN ln -s /usr/local/zookeeper-3.4.10 /usr/local/zookeeper;\
 chown -R root:root /usr/local/zookeeper-3.4.10;\
